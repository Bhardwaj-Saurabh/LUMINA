...
## Decided architecture & deployment (see docs/ARCHITECTURE.md for full rationale)
...
- Module layout for both services (hexagonal ports & adapters, tool registry with `forDepth()` filtering, repository per collection, job state machine) is specified in docs/ARCHITECTURE.md §2 — follow it.
